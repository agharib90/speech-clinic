    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div class="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
                    ...
                    <p class="text-xl font-bold text-blue-800 dark:text-blue-100">{{ number_format($baseSalary, 2) }}</p>
                </div>
                <div class="bg-green-50 dark:bg-green-900 p-4 rounded-lg">
                    <p class="text-sm text-green-600 dark:text-green-300">عمولات الجلسات</p>
                    <p class="text-xl font-bold text-green-800 dark:text-green-100">{{ number_format($totalEarnings, 2) }}</p>
                </div>
                <div class="bg-yellow-50 dark:bg-yellow-900 p-4 rounded-lg">
                    <p class="text-sm text-yellow-600 dark:text-yellow-300">بدلات إضافية</p>
                    <p class="text-xl font-bold text-yellow-800 dark:text-yellow-100">{{ number_format($additions, 2) }}</p>
                </div>
                <div class="bg-red-50 dark:bg-red-900 p-4 rounded-lg">
                    <p class="text-sm text-red-600 dark:text-red-300">خصومات</p>
                    <p class="text-xl font-bold text-red-800 dark:text-red-100">{{ number_format($deductions, 2) }}</p>
                </div>
            </div>
            ...
            <div class="mt-6 bg-gray-100 dark:bg-gray-700 p-4 rounded-lg text-left">
                ...
                <p class="text-4xl font-extrabold text-gray-900 dark:text-white">{{ number_format($netSalary, 2) }} <span class="text-lg">ر.س</span></p>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 font-bold dark:text-white">{{ number_format($earning->amount, 2) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            ...
            <form action="{{ route('payroll.addRecord') }}" method="POST" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <button type="submit" class="bg-gray-800 hover:bg-gray-900 text-white py-2 rounded-lg">إضافة</button>
            </form>
        </div>
