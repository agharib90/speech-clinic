<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow mb-6">
        <div class="flex justify-between items-start mb-8">
            ...
            <a href="{{ route('invoices.pdf', $invoice) }}" target="_blank" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg">تحميل PDF</a>
        </div>
        ...
        <table class="w-full text-right mb-6">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-4 py-2 text-sm text-gray-500">الوصف</th>
                    <th class="px-4 py-2 text-sm text-gray-500">الكمية</th>
                    <th class="px-4 py-2 text-sm text-gray-500">سعر الوحدة</th>
                    <th class="px-4 py-2 text-sm text-gray-500">الإجمالي</th>
                </tr>
            </thead>
            ...
            <tfoot>
                <tr class="border-t-2 dark:border-gray-600">
                    ...
                    <td class="px-4 py-3 font-extrabold text-xl text-blue-600 dark:text-blue-400">{{ number_format($invoice->total, 2) }} ر.س</td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="border-t dark:border-gray-700 pt-6">
            <h3 class="font-bold mb-4 text-gray-800 dark:text-gray-200">سجل الدفعات (المبلغ المدفوع: {{ number_format($invoice->payments->sum('amount'), 2) }})</h3>
            ...
            <ul class="space-y-2 mb-4">
                ...
                <li class="flex justify-between bg-gray-50 dark:bg-gray-700 p-3 rounded">
                    ...
                    <span class="font-bold text-green-600">{{ number_format($payment->amount, 2) }} ر.س</span>
                </li>
                @endforeach
            </ul>
            @endif
            ...
            <form action="{{ route('invoices.payments', $invoice) }}" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg h-fit">تسجيل دفعة</button>
            </form>
            @endif
        </div>
    </div>
