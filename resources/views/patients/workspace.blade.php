<x-app-layout>
    ...
    @php
        ...
        $operationalReady = in_array($workspace['clinical']['key'], ['handoff_ready', 'operational', 'legacy_plan'], true);
        ...
        $alertSummaryTone = match (true) {
            $workspace['alerts']->contains(fn ($alert) => ($alert['tone'] ?? null) === 'danger') => 'danger',
            $workspace['alerts']->contains(fn ($alert) => ($alert['tone'] ?? null) === 'warning') => 'warning',
            $workspace['alerts']->contains(fn ($alert) => ($alert['tone'] ?? null) === 'primary') => 'primary',
            default => 'success',
        };
        ...
        $workspaceSections = collect([
            ...
            ['key' => 'clinical', 'label' => 'المسار السريري', 'visible' => $workspace['clinical']['key'] !== 'legacy_plan'],
            ['key' => 'plan', 'label' => 'الخطة', 'visible' => $operationalReady && auth()->user()->can('manage patient service plans')],
            ['key' => 'finance', 'label' => 'المالية والفواتير', 'visible' => $operationalReady && (auth()->user()->can('view finance') || auth()->user()->can('manage invoices'))],
            ['key' => 'appointments', 'label' => 'المواعيد', 'visible' => $operationalReady && auth()->user()->can('view appointments')],
            ['key' => 'therapy', 'label' => 'البرامج العلاجية', 'visible' => auth()->user()->can('view therapy')],
            ['key' => 'guardian', 'label' => 'ولي الأمر', 'visible' => true],
            ['key' => 'activity', 'label' => 'النشاط', 'visible' => true],
        ])->where('visible')->values();
        $availableSectionKeys = $workspaceSections->pluck('key')->all();
        $errorPanel = $errors->any() ? old('workspace_panel') : null;
        $errorSection = $panelSections[$errorPanel] ?? null;
        $requestedSection = request()->query('section');
        $initialSection = in_array($errorSection, $availableSectionKeys, true)
            ? $errorSection
            : (in_array($requestedSection, $availableSectionKeys, true) ? $requestedSection : 'overview');
    @endphp

    <div class="patient-workspace -m-4 min-h-screen p-4 sm:-m-6 sm:p-6 lg:-m-8 lg:p-8" x-data="patientWorkspace({
        initialSection: {{ Illuminate\Support\Js::from($initialSection) }},
        initialPanel: {{ Illuminate\Support\Js::from($initialPanel) }},
        sections: {{ Illuminate\Support\Js::from($availableSectionKeys) }},
        panelSections: {{ Illuminate\Support\Js::from($panelSections) }},
    })">
        <div class="mx-auto max-w-[1500px] space-y-5 pb-8">
            ...
            <section aria-labelledby="quick-actions-title">
                ...
                <div class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
                    @can('manage invoices')
                        @if($patient->is_active && $operationalReady)
                            <button type="button" @click="openPanel('invoice')" data-workspace-open="finance:invoice" class="workspace-action w-full">
                    ...
                    @if($operationalReady && auth()->user()->can('manage invoices') && auth()->user()->can('view finance') && $workspace['latestOutstandingInvoice'])
                        <button type="button" @click="openPanel('payment')" data-workspace-open="finance:payment" class="workspace-action w-full">
                    ...
                    @can('manage patient service plans')
                        @if($operationalReady && $plan && $plan->status === \App\Models\PatientServicePlan::STATUS_ACTIVE && $workspace['unallocatedPayments']->isNotEmpty())
                            <button type="button" @click="openPanel('allocation')" data-workspace-open="plan:allocation" class="workspace-action w-full">
                        ...
                        @elseif($operationalReady)
                            <button type="button" @click="openPanel('plan')" data-workspace-open="plan:plan" class="workspace-action w-full">
                    ...
                    @if($operationalReady && auth()->user()->can('view appointments') && auth()->user()->can('create appointments'))
                        <button type="button" @click="openPanel('appointment')" data-workspace-open="appointments:appointment" class="workspace-action w-full">
            ...
            <section class="grid grid-cols-1 gap-4 xl:grid-cols-3">
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <h2 class="mt-1 text-lg font-bold text-text">
                                @if($plan)
                                    <a href="{{ route('patients.workspace', [$patient, 'section' => 'plan']) }}" @click.prevent="openSection('plan', true)" class="clinic-entity-link">خطة #{{ $plan->id }} · {{ \App\Models\PatientServicePlan::STATUS_LABELS[$plan->status] ?? $plan->status }}</a>
                                @else
                ...
                <article class="workspace-card p-5">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <span class="flex h-10 w-10 items-center justify-center rounded-lg border {{ $alertClasses[$alertSummaryTone] ?? $alertClasses['primary'] }}">
                            @if($alertSummaryTone === 'success')
                            ...
                            @elseif($alertSummaryTone === 'warning')
                            ...
                            @elseif($alertSummaryTone === 'danger')
            ...
            <section class="workspace-recommendation rounded-lg border border-primary/30 p-5 sm:p-6">
                <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
                    ...
                    @if($workspace['recommendation']['panel'])
                        <button type="button" @click="openPanel('{{ $workspace['recommendation']['panel'] }}')" data-workspace-open="{{ $panelSections[$workspace['recommendation']['panel']] ?? 'overview' }}:{{ $workspace['recommendation']['panel'] }}" class="clinic-btn-primary shrink-0">ابدأ الخطوة</button>
                    @elseif($workspace['recommendation']['url'])
                        <a href="{{ $workspace['recommendation']['url'] }}" class="clinic-btn-primary shrink-0">ابدأ الخطوة</a>
                    @endif
                </div>

                @if($workspace['clinical']['key'] !== 'legacy_plan')
                <div class="mt-7 grid grid-cols-3 gap-y-5 sm:grid-cols-7" aria-label="مراحل سير الحالة">
                    @foreach($workspace['workflow'] as $index => $stage)
                        <div class="relative flex flex-col items-center text-center">
                            ...
                            @endif
                            <span class="relative z-10 flex h-8 w-8 items-center justify-center rounded-full border text-xs font-bold {{ $stage['complete'] ? 'border-success bg-success text-success-contrast' : (($stage['current'] ?? false) ? 'border-primary bg-primary-soft text-primary' : 'border-surface-border bg-surface-muted text-text-subtle') }}">
                                {{ $stage['complete'] ? '✓' : $index + 1 }}
                            </span>
                            <span class="mt-2 text-xs font-medium {{ $stage['complete'] ? 'text-success' : (($stage['current'] ?? false) ? 'text-primary' : 'text-text-muted') }}">{{ $stage['label'] }}</span>
                        </div>
                    @endforeach
                </div>
                @endif
            ...
            <div class="sticky top-0 z-20 rounded-lg border border-surface-border bg-surface-elevated p-2 shadow-sm" data-workspace-section-navigation>
                <div class="flex items-center gap-2 overflow-x-auto" role="tablist" aria-label="أقسام مساحة عمل الحالة">
                    @foreach($workspaceSections as $workspaceSection)
                        <button type="button"
                                role="tab"
                                data-workspace-section-tab="{{ $workspaceSection['key'] }}"
                                @click="openSection('{{ $workspaceSection['key'] }}')"
                                :aria-selected="section === '{{ $workspaceSection['key'] }}'"
                                :class="section === '{{ $workspaceSection['key'] }}' ? 'border-primary bg-primary-soft text-primary' : 'border-transparent text-text-muted hover:bg-surface-muted hover:text-text'"
                                class="min-h-10 shrink-0 rounded-md border px-3 py-2 text-sm font-semibold transition">
                            {{ $workspaceSection['label'] }}
                        </button>
                    @endforeach
                    <span class="mr-auto shrink-0 rounded-full border px-3 py-1 text-xs font-semibold"
                          :class="hasUnsavedChanges ? 'border-warning/30 bg-warning-soft text-warning' : 'border-success/30 bg-success-soft text-success'"
                          data-workspace-dirty-indicator>
                        <span x-show="hasUnsavedChanges">لديك تغييرات غير محفوظة</span>
                        <span x-show="!hasUnsavedChanges">كل التغييرات محفوظة</span>
                    </span>
                </div>
            </div>

            <div x-ref="sectionContent" class="scroll-mt-20">
            <section class="workspace-card p-5 sm:p-6" data-workspace-section="overview" x-show="section === 'overview'" x-cloak>
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                    <div><p class="text-xs font-semibold text-primary">نظرة عامة</p><h2 class="mt-1 text-xl font-bold text-text">الوضع الحالي للحالة</h2></div>
                    <span class="w-fit rounded-full border border-primary/30 bg-primary-soft px-3 py-1 text-xs font-semibold text-primary">{{ $workspace['clinical']['title'] }}</span>
                </div>
                <dl class="mt-5 grid gap-3 sm:grid-cols-3">
                    <div class="rounded-lg bg-surface-muted p-4"><dt class="text-xs text-text-muted">الخطة الحالية</dt><dd class="mt-1 font-semibold text-text">{{ $plan ? 'خطة #'.$plan->id : 'لا توجد خطة نشطة' }}</dd></div>
                    <div class="rounded-lg bg-surface-muted p-4"><dt class="text-xs text-text-muted">الموعد القادم</dt><dd class="mt-1 font-semibold text-text">{{ $workspace['upcomingAppointments']->first()?->scheduled_at?->format('Y-m-d H:i') ?? 'لا يوجد موعد قادم' }}</dd></div>
                    <div class="rounded-lg bg-surface-muted p-4"><dt class="text-xs text-text-muted">آخر نشاط</dt><dd class="mt-1 font-semibold text-text">{{ $workspace['recentActivity']->first()['title'] ?? 'لا يوجد نشاط مسجل' }}</dd></div>
                </dl>
            </section>

            @if($workspace['clinical']['key'] !== 'legacy_plan')
            <div data-workspace-section="clinical" x-show="section === 'clinical'" x-cloak>
            @include('patients._clinical-workflow', [
                'clinical' => $workspace['clinical'],
                'evaluation' => $workspace['currentEvaluation'],
                'clinicalPlan' => $workspace['clinicalPlan'],
                'services' => $workspace['services'],
                'assignment' => $workspace['openEvaluationAssignment'],
                'eligibleClinicians' => $workspace['eligibleClinicians'],
            ])
            </div>
            @endif

            @if($operationalReady && auth()->user()->can('manage patient service plans'))
                <section id="workspace-plan" class="workspace-card scroll-mt-4 p-5 sm:p-6" data-workspace-section="plan" x-show="section === 'plan'" x-cloak>
                    <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                    ...
                    @if($plan)
                        ...
                        <div class="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-surface-muted px-4 py-3 text-sm">
                            ...
                            @if($plan->status === \App\Models\PatientServicePlan::STATUS_DRAFT
                                && ($plan->clinical_evaluation_id === null || $plan->isClinicallyApproved()))
                                <form method="POST" action="{{ URL::signedRoute('patient-service-plans.activate', ['patientServicePlan' => $plan, 'workspace_patient' => $patient->id]) }}" data-workspace-direct-action>@csrf<input type="hidden" name="workspace" value="1"><input type="hidden" name="workspace_panel" value="plan"><input type="hidden" name="workspace_section" value="plan"><button class="clinic-btn-primary">تفعيل الخطة</button></form>
                            @endif
                        </div>
                            ...
                            <div id="workspace-allocation" class="mt-5 scroll-mt-4" x-show="panel === 'allocation' || {{ $workspace['unallocatedPayments']->isNotEmpty() ? 'true' : 'false' }}" x-cloak>
                                ...
                                <div class="mt-3 grid gap-3 lg:grid-cols-2">
                                    @forelse($workspace['unallocatedPayments'] as $payment)
                                        <form method="POST" action="{{ URL::signedRoute('patient-service-plans.allocate-payment', ['patientServicePlan' => $plan, 'workspace_patient' => $patient->id]) }}" class="flex flex-col justify-between gap-3 rounded-lg border border-warning/30 bg-warning-soft p-4 sm:flex-row sm:items-center" data-workspace-direct-action>@csrf<input type="hidden" name="workspace" value="1"><input type="hidden" name="workspace_panel" value="allocation"><input type="hidden" name="workspace_section" value="plan"><input type="hidden" name="invoice_payment_id" value="{{ $payment->id }}"><div><p class="font-semibold text-text">{{ number_format((float) $payment->amount, 2) }} ج.م</p><p class="text-xs text-text-muted">{{ $payment->invoice->invoice_number }} · {{ $payment->payment_date }}</p></div><button class="clinic-btn-primary">تخصيص للخطة</button></form>
                                    @empty
                        ...
                        @endif
                    @else
                        <div class="mt-4 flex flex-col items-start gap-3 rounded-lg border border-warning/30 bg-warning-soft p-4"><p class="text-sm text-warning">أنشئ مسودة الخطة هنا دون مغادرة سياق {{ $patient->name }}.</p><button type="button" class="clinic-btn-primary" @click="togglePanel('plan')">إنشاء خطة</button></div>
            ...
            @if($operationalReady && (auth()->user()->can('view finance') || auth()->user()->can('manage invoices')))
                <section id="workspace-finance" class="workspace-card scroll-mt-4 p-5 sm:p-6" data-workspace-section="finance" x-show="section === 'finance'" x-cloak>
                    <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                        ...
                        <div class="flex flex-wrap gap-2">@can('manage invoices')<button type="button" class="clinic-btn-primary" @click="togglePanel('invoice')">إنشاء فاتورة</button>@endcan @if(auth()->user()->can('manage invoices') && auth()->user()->can('view finance') && $workspace['payableInvoices']->isNotEmpty())<button type="button" class="clinic-btn-secondary" @click="togglePanel('payment')">تسجيل دفعة</button>@endif</div>
                    </div>
            ...
            @if($operationalReady && auth()->user()->can('view appointments'))
                <section id="workspace-appointments" class="workspace-card scroll-mt-4 p-5 sm:p-6" data-workspace-section="appointments" x-show="section === 'appointments'" x-cloak>
                    <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p class="text-xs font-semibold text-primary">المواعيد</p><h2 class="mt-1 text-xl font-bold text-text">جدول {{ $patient->name }}</h2></div><div class="flex gap-2">@can('create appointments')<button type="button" class="clinic-btn-primary" @click="togglePanel('appointment')">حجز موعد</button>@endcan<a href="{{ route('appointments.index', ['patient_id' => $patient->id]) }}" class="clinic-btn-secondary">الجدول العام</a></div></div>
                    ...
                    <div class="mt-5 divide-y divide-surface-border rounded-lg border border-surface-border px-4">
                        @forelse($workspace['patientAppointments'] as $appointment)
                            <div class="flex flex-col justify-between gap-3 py-4 sm:flex-row sm:items-center">
                                <div>
                                    <p class="font-semibold text-text">{{ $appointment->patientServicePlanItem?->service?->name ?? $appointment->sessionType?->name ?? 'خدمة غير محددة' }}</p>
                                    <p class="mt-1 text-xs text-text-muted">{{ $appointment->scheduled_at->format('Y-m-d H:i') }} · {{ $appointment->therapist?->name ?? 'أخصائي غير محدد' }}</p>
                                </div>
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="rounded-full border border-surface-border px-2 py-1 text-xs text-text-muted">{{ $appointment->status }}</span>
                                    @if(! $appointment->patient_service_plan_item_id && $appointment->legacy_booking_reason)
                                        <span class="rounded-full bg-warning-soft px-2 py-1 text-xs text-warning">حجز استثنائي</span>
                                    @elseif($appointment->patient_service_plan_item_id)
                                        @php($completionState = $appointment->service_completion_state)
                                        @if($appointment->checkin)
                                            <span class="rounded-full bg-success-soft px-2 py-1 text-xs text-success">تم تأكيد الحضور</span>
                                        @endif
                                        <span class="rounded-full bg-surface-muted px-2 py-1 text-xs text-text-muted">{{ $completionState['label'] }}</span>
                                        @if($completionState['can_complete'])
                                            <form action="{{ route('appointments.complete-service', $appointment) }}" method="POST" data-workspace-direct-action>
                                                @csrf
                                        ...
                                        @endif
                                    @elseif($appointment->financially_confirmed_at)
                                        <span class="rounded-full bg-success-soft px-2 py-1 text-xs text-success">مؤكد ماليًا</span>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <p class="py-8 text-center text-sm text-text-muted">لا توجد مواعيد مسجلة.</p>
                        @endforelse
                    </div>
                </section>
            @endif
            ...
            @can('view therapy')
                <section class="workspace-card p-5" data-workspace-section="therapy" x-show="section === 'therapy'" x-cloak><h2 class="text-lg font-bold text-text">البرامج العلاجية</h2><div class="mt-4 divide-y divide-surface-border">@forelse($patient->therapyPrograms as $program)<div class="flex flex-col justify-between gap-2 py-3 first:pt-0 sm:flex-row sm:items-center"><div>@can('view', $program)<a href="{{ route('programs.show', $program) }}" class="clinic-entity-link font-semibold">{{ $program->name }}</a>@else<span class="font-semibold text-text">{{ $program->name }}</span>@endcan<p class="mt-1 text-xs text-text-muted">{{ $program->disorder_type ?: 'نوع الاضطراب غير محدد' }} · {{ $program->therapist?->name ?? 'أخصائي غير محدد' }}</p></div><div class="text-xs text-text-muted">{{ $program->status }} · {{ $program->sessions_count }} جلسة · {{ $program->attachments_count }} مرفق</div></div>@empty<p class="py-8 text-center text-sm text-text-muted">لا توجد برامج علاجية مسجلة.</p>@endforelse</div></section>
            @endcan

            <section class="workspace-card p-5" data-workspace-section="guardian" x-show="section === 'guardian'" x-cloak><h2 class="text-lg font-bold text-text">ولي الأمر والتواصل</h2>@if($patient->guardian)<dl class="mt-4 space-y-3 text-sm"><div><dt class="text-text-muted">الاسم</dt><dd class="mt-1"><a href="{{ route('guardians.show', $patient->guardian) }}" class="clinic-entity-link font-semibold">{{ $patient->guardian->name }}</a></dd></div><div><dt class="text-text-muted">الهاتف</dt><dd class="mt-1 font-semibold text-text" dir="ltr">{{ $patient->guardian->phone ?: 'غير مسجل' }}</dd></div>@if($patient->guardian->phone2)<div><dt class="text-text-muted">هاتف آخر</dt><dd class="mt-1 text-text" dir="ltr">{{ $patient->guardian->phone2 }}</dd></div>@endif @if($patient->guardian->email)<div><dt class="text-text-muted">البريد</dt><dd class="mt-1 break-all text-text">{{ $patient->guardian->email }}</dd></div>@endif @if($patient->guardian->address)<div><dt class="text-text-muted">العنوان</dt><dd class="mt-1 text-text">{{ $patient->guardian->address }}</dd></div>@endif</dl>@else<p class="mt-4 text-sm text-text-muted">لا توجد بيانات ولي أمر مرتبطة.</p>@endif</section>

            <section class="workspace-card p-5" data-workspace-section="activity" x-show="section === 'activity'" x-cloak><h2 class="text-lg font-bold text-text">النشاط الأخير</h2><div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">@forelse($workspace['recentActivity'] as $activity)<div class="flex gap-3"><span class="mt-1 h-2.5 w-2.5 shrink-0 rounded-full {{ $activity['type'] === 'payment' ? 'bg-success' : 'bg-primary' }}"></span><div class="min-w-0 flex-1"><p class="truncate text-sm font-medium text-text">{{ $activity['title'] }}</p><div class="mt-1 flex justify-between gap-3 text-[11px] text-text-subtle"><span>{{ $activity['detail'] }}</span><time>{{ $activity['at']->diffForHumans() }}</time></div></div></div>@empty<p class="py-8 text-center text-sm text-text-muted">لا يوجد نشاط مسجل بعد.</p>@endforelse</div></section>
            </div>
        </div>
    </div>
</x-app-layout>
